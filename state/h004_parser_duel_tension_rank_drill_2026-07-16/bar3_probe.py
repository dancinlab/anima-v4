...
def main() -> int:
    ...
    torch_, M, cfg, Struct = T._load(smoke=True)                 # d=64, L=2
    ...
    cpt_win = T._cpt_windows(T._nsmc_lines(2000), 128, torch_)
    print(f"bar3-probe: d=64/L2 on {device}, CPT 200 + drill 1500 (fixed objective) …", flush=True)
    out = {"device": device, "arms": {}}
    for arm in ("C-perm", "A-duel"):
        model, ce_ans = T.train_arm(arm, 0, (torch_, M, cfg, Struct), {"f2": f2, "f1": f1},
                                    drill, cpt_win, (200, 1500), device, tag=None)
        rec = {
    ...
    with open(os.path.join(_HERE, "bar3_probe_result.json"), "w") as fh:
