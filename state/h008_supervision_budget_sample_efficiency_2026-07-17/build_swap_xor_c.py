#!/usr/bin/env python3
"""H_008 mech-supervision-budget G-0b-C — SWAP-XOR-C panel + budget constructor + full audit ($0).
...
byte families 사장~부장(장)/학생~동생(생) + NSMC corpus semantics). SWAP-XOR-C (the seed §3.1 spec, after SWAP-XOR-B's A7 was found to force a PARITY TRAP) fixes this with:

  1. 48 DISTINCT non-corpus synthetic syllables → 24 lexemes (each syllable used in exactly ONE
     lexeme), class an ARBITRARY per-lexeme partition. A7′ = no cross-lexeme syllable sharing (unanswered
     lexeme class = 0 bits) + no syllable/lexeme in CPT — NOT "class is no function of bytes" (that forced
     SWAP-XOR-B's 24-paired parity conjunction, unlearnable by SGD; G-1.5a K1-VOID-FOR-CAUSE).
...
Audit suite (exit-0-blocking, ALL recomputed, never inherited):
  ...
  A4 length-parity · A5 swap(x) byte-aligned · A6 supervision-leak ban · A7′ disjoint-noncorpus
  (48 distinct syllables, no cross-lexeme sharing, none in CPT) ·
...
Run:  python3 state/h008_supervision_budget_sample_efficiency_2026-07-17/build_swap_xor_c.py
Exit: 0 iff every audit PASS. Writes swapc_f2.json / swapc_drill.json / swapc_budgets.json / swapc_pools.json.
"""
...
def build_pools():
    """SWAP-XOR-C (seed §3.1 as actually specified): 48 DISTINCT non-corpus syllables → 24 lexemes, each
    from 2 syllables used in NO other lexeme (lexeme i = syl[2i]+syl[2i+1]). Class is an ARBITRARY per-lexeme
    partition (interleaved 12/12 so lexeme-index does not correlate with class order). Because every syllable
    is unique to one lexeme, an UNANSWERED lexeme's class is EXACTLY zero bits (A7′) — the property that
    matters — and class is a per-lexeme lookup SGD learns (no parity conjunction; the SWAP-XOR-B defect)."""
    nsmc = open(_NSMC, encoding="utf-8").read() if os.path.exists(_NSMC) else ""
    syl = _rare_syllables(nsmc, 48)                              # 48 DISTINCT
    lex = [syl[2 * i] + syl[2 * i + 1] for i in range(24)]       # each syllable in exactly one lexeme
    # arbitrary class: interleave so index parity ≠ class (deterministic, balanced 12/12)
    honored = [lex[i] for i in range(24) if (i % 2 == 0)]
    plain = [lex[i] for i in range(24) if (i % 2 == 1)]
    assert len(set(honored + plain)) == 24, "lexemes not distinct"
    assert len({s for n in lex for s in (n[0], n[1])}) == 48, "syllables not all distinct"
    for n in honored + plain:
        assert len(n.encode("utf-8")) == 6, f"{n} not 6 bytes"
    P = [n[0] for n in lex]; Q = [n[1] for n in lex]            # position-1 / position-2 syllables (all distinct)
...
def audit(drill, f2, budgets):
    ...
    # A7′ (SWAP-XOR-C, the property that MATTERS — replaces the parity-forcing "syllable→class=0.5"):
    #   (a) NO cross-lexeme syllable sharing ⇒ an unanswered lexeme's class is EXACTLY zero bits (kills the
    #       H_007 side channel: nothing about a held-out lexeme's class leaks from any other lexeme); AND
    #   (b) no syllable / no lexeme in the CPT corpus (kills the NSMC distributional channel).
    # We do NOT require class to be a null function of bytes — that forced the SWAP-XOR-B parity trap.
    all_syl = [s for n in LEX for s in (n[0], n[1])]
    from collections import Counter as _C
    shared = [s for s, c in _C(all_syl).items() if c > 1]
    syl_in_cpt = [s for s in set(all_syl) if _NSMC_TEXT and s in _NSMC_TEXT]
    lex_in_cpt = [n for n in LEX if _NSMC_TEXT and n in _NSMC_TEXT]
    chk("A7prime_disjoint_noncorpus",
        not shared and not syl_in_cpt and not lex_in_cpt,
        f"cross-lexeme-shared-syllables={shared} · syllables-in-CPT={syl_in_cpt} · lexemes-in-CPT={lex_in_cpt} "
        f"· 48-distinct={len(set(all_syl))==48}")
...
def main():
    ...
    print("=" * 80)
    print("H_008 G-0b-C — SWAP-XOR-C panel + budget constructor + audit")
    print("=" * 80)
    print(f"lexicon: 48-distinct non-corpus synthetic (POS1={''.join(POS1)} POS2={''.join(POS2)})")
    ...
    print()
    print("G-0b VERDICT:", "PASS — SWAP-XOR-C admissible (A7′, parity trap removed), freeze-blocked on G-1.5a-C"
    ...
    if res["pass"]:
        ...
        json.dump({"panel": f2, "meta": {**meta, "kind": "f2_verdict_ood", "n": len(f2)}},
                  open(os.path.join(_HERE, "swapc_f2.json"), "w"), ensure_ascii=False, indent=1)
        json.dump({"drill": drill, "meta": {**meta, "kind": "drill_coverage_complete", "n": len(drill)}},
                  open(os.path.join(_HERE, "swapc_drill.json"), "w"), ensure_ascii=False, indent=1)
        json.dump({k: v for k, v in budgets.items()},
                  open(os.path.join(_HERE, "swapc_budgets.json"), "w"), ensure_ascii=False, indent=1)
        json.dump({"honored": HON, "plain": PLAIN, "pos1": POS1, "pos2": POS2},
                  open(os.path.join(_HERE, "swapc_pools.json"), "w"), ensure_ascii=False, indent=1)
        print("wrote swapc_f2.json / swapc_drill.json / swapc_budgets.json / swapc_pools.json")
